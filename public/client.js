(() => {
  ...
  function bindEvents() {
    ...
    els.remoteVideo.addEventListener('loadedmetadata', resizeRemoteOverlay);
  ...
  async function loadConfig() {
    try {
      ...
      if (data && Array.isArray(data.iceServers)) state.config = data;
      log('config', data);
    } catch (err) {
  ...
  async function loadCameras() {
    try {
      ...
      log('camera enumerate failed', err);
    }
  }
  ...
  async function startCamera() {
    try {
      ...
      log('local tracks', newStream.getTracks().map(t => `${t.kind}:${t.readyState}`));
      if (state.joined && state.peerTargetId) await renegotiatePeer(true);
    } catch (err) {
      console.error(err);
  ...
  function setRoomStatus(text) { els.roomStatus.textContent = text; }
  function setSyncStatus(text) { els.syncStatus.textContent = text; }
  function setMode(text) { els.modeBadge.textContent = text; }
  ...
  async function joinRoom() {
    ...
    try {
      ...
      state.side = side;
      updateBadges();
      sendSocket({ type: 'join-room', roomId, playerId: state.playerId, name: playerName, side });
      ...
      log('join sent', roomId, state.playerId, playerName);
    } catch (err) {
      console.error(err);
      setSyncStatus('Connection failed.');
  ...
  function copyRoomLink() {
    ...
    navigator.clipboard.writeText(url.toString()).then(() => setSyncStatus('Room link copied.')).catch(() => setSyncStatus('Could not copy link.'));
  }
  ...
  async function onSocketMessage(event) {
    ...
    log('socket message', message.type, message);
    switch (message.type) {
      case 'room-joined':
        state.joined = true;
      ...
      case 'peer-left':
        ...
        updateRoster(state.roster.filter((p) => p.playerId !== message.playerId));
        break;
      case 'state-update':
        if (message.state) {
  ...
  function updateRoster(roster) {
    state.roster = roster;
    els.roster.textContent = roster.length ? `Players: ${roster.map((player) => `${player.name} (${player.side})`).join(', ')}` : 'Players: none';
  }
  ...
  async function attachToExistingPeer(roster) {
    ...
    if (!peer) return updatePeerBadge(null);
    state.peerTargetId = peer.playerId;
    updatePeerBadge(peer.playerId);
    if (!state.peerConnection) await ensurePeerConnection(peer.playerId);
    log('attach existing peer', peer.playerId);
  }
  ...
  async function attachToKnownPeer(peerId, shouldOffer) {
    ...
    if (shouldOffer) await makeOffer();
    log('attach known peer', peerId, 'offer?', shouldOffer);
  }

  async function ensurePeerConnection(peerId = state.peerTargetId) {
    if (state.peerConnection && state.peerTargetId === peerId) return state.peerConnection;
    ...
    pc.onicecandidate = (event) => {
      ...
      log('send candidate', event.candidate.type || 'cand');
      sendSocket({ type: 'signal', targetPlayerId: state.peerTargetId, signal: { candidate: event.candidate } });
    };

    pc.ontrack = (event) => {
      log('ontrack', event.track.kind, event.streams?.length || 0);
      const incomingStream = state.remoteStream || new MediaStream();
      const incomingTracks = event.streams && event.streams[0] ? event.streams[0].getTracks() : [event.track];
      incomingTracks.forEach((track) => {
        const alreadyAdded = incomingStream.getTracks().some((t) => t.id === track.id);
        if (!alreadyAdded) incomingStream.addTrack(track);
      });
      state.remoteStream = incomingStream;
      els.remoteVideo.srcObject = incomingStream;
      ...
      els.remoteBadge.textContent = state.peerTargetId ? `Connected target: ${state.peerTargetId}` : 'Peer connected';
      resizeRemoteOverlay();
    };

    pc.onconnectionstatechange = () => {
      const status = pc.connectionState || 'unknown';
      log('pc connection', status);
      if (status === 'connected') setSyncStatus('Peer connected.');
      else if (status === 'connecting') setSyncStatus('Connecting peer...');
      else if (status === 'disconnected') setSyncStatus('Peer disconnected.');
      else if (status === 'failed') setSyncStatus('Peer connection failed.');
      else if (status === 'closed') setSyncStatus('Peer closed.');
    };
    ...
    if (state.localStream) addLocalTracksToPeer(pc);
  ...
  function addLocalTracksToPeer(pc) {
    ...
    const existingTrackIds = new Set(pc.getSenders().map((sender) => sender.track && sender.track.id).filter(Boolean));
  ...
  async function renegotiatePeer(shouldOffer) {
    ...
    await ensurePeerConnection(state.peerTargetId);
    addLocalTracksToPeer(state.peerConnection);
    if (shouldOffer) await makeOffer();
  }

  async function makeOffer() {
    try {
      const pc = await ensurePeerConnection(state.peerTargetId);
      addLocalTracksToPeer(pc);
      if (!state.peerTargetId) return;
      const offer = await pc.createOffer({ offerToReceiveAudio: true, offerToReceiveVideo: true });
      await pc.setLocalDescription(offer);
      log('send offer');
      sendSocket({ type: 'signal', targetPlayerId: state.peerTargetId, signal: { description: pc.localDescription } });
    } catch (err) {
      console.error('Offer failed:', err);
      setSyncStatus('Offer failed.');
    }
  }
  ...
  async function handleSignalMessage(message) {
    ...
    try {
      if (signal.description) {
        ...
        log('recv description', description.type);
        await pc.setRemoteDescription(description);
        if (state.pendingIceCandidates.length) {
          for (const candidate of state.pendingIceCandidates) {
            try { await pc.addIceCandidate(candidate); } catch (candidateErr) { log('queued candidate failed', candidateErr); }
          }
          state.pendingIceCandidates = [];
        }
        ...
        else state.pendingIceCandidates.push(candidate);
      }
    } catch (err) {
      console.error('Signal handling failed:', err);
      setSyncStatus('Signal handling failed.');
    }
  }
  ...
  function syncMatchTools() {
    const nextState = { match: { selfLp: Number(els.selfLpInput.value || 0), opponentLp: Number(els.opponentLpInput.value || 0), phase: els.phaseSelect.value } };
    state.sharedState = { ...state.sharedState, ...nextState };
    sendSocket({ type: 'state-update', state: state.sharedState });
  ...
  function searchSampleCards(query) {
    ...
    return haystack.filter((card) => String(card.name || '').toLowerCase().includes(q)).slice(0, 8).map(normalizeCard);
  }
  ...
  function renderResults(results) {
    ...
    results.forEach((card) => {
      ...
      node.addEventListener('click', () => renderSelectedCard(card));
  ...
  function renderSelectedCard(card) {
    ...
    wrap.appendChild(img); wrap.appendChild(title); wrap.appendChild(meta); if (stats.textContent) wrap.appendChild(stats); wrap.appendChild(desc);
  ...
  async function onRemoteVideoClick(event) {
    if (!els.remoteVideo.srcObject || !els.remoteVideo.videoWidth || !els.remoteVideo.videoHeight) return;
    ...
    try {
      ...
      captureCtx.drawImage(els.remoteVideo, sx, sy, cropWidth, cropHeight, 0, 0, cropWidth, cropHeight);
  ...
  async function runOcr(canvas) {
    ...
    const result = await window.Tesseract.recognize(canvas, 'eng', {
      logger: (msg) => { if (msg.status) setSyncStatus(`OCR: ${msg.status}`); }
    });
  ...
  function extractLikelyCardName(text) {
    ...
    const candidates = lines.filter((line) => line.length >= 3 && line.length <= 40).sort((a, b) => a.length - b.length);
    if (candidates.length) cleaned = candidates[0];
    cleaned = cleaned.replace(/\b(ATK|DEF|SPELL|TRAP|MONSTER|EFFECT|CARD|LINK|XYZ|SYNCHRO|FUSION)\b/gi, '').replace(/\b\d+\b/g, ' ').replace(/\s+/g, ' ').trim();
