(() => {
  ...
  async function startCamera() {
    try {
      ...
      if (state.joined && state.peerTargetId) {
  ...
  async function attachToExistingPeer(roster) {
    ...
    if (!state.peerConnection) {
      await ensurePeerConnection(peer.playerId);
    }
  }
  ...
  async function ensurePeerConnection(peerId = state.peerTargetId) {
    if (state.peerConnection && state.peerTargetId === peerId) {
    ...
    pc.ontrack = (event) => {
      ...
      incomingTracks.forEach((track) => {
        const alreadyAdded = incomingStream
          .getTracks()
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (status === 'connected') {
        ...
        setSyncStatus('Peer disconnected.');
      } else if (status === 'failed') {
        setSyncStatus('Peer connection failed.');
      } else if (status === 'closed') {
        setSyncStatus('Peer closed.');
      }
    };

    pc.oniceconnectionstatechange = () => {
      const iceStatus = pc.iceConnectionState || 'unknown';
      console.log('ICE state:', iceStatus);
    };
        ...
        if (state.pendingIceCandidates.length) {
          for (const candidate of state.pendingIceCandidates) {
            try {
              ...
              console.warn('Queued ICE candidate failed:', candidateErr);
            }
          }
          state.pendingIceCandidates = [];
        }
  ...
  function searchSampleCards(query) {
    const haystack = Array.isArray(window.SAMPLE_CARDS)
      ? window.SAMPLE_CARDS
      : [];
  ...
  function renderResults(results) {
    ...
    results.forEach((card) => {
      const node =
        els.resultTemplate.content.firstElementChild.cloneNode(true);
  ...
  function buildStatsLine(card) {
    ...
    if (card.atk !== '' && card.atk !== null && card.atk !== undefined) {
      parts.push(`ATK ${card.atk}`);
    }

    if (card.def !== '' && card.def !== null && card.def !== undefined) {
  ...
  async function onRemoteVideoClick(event) {
    if (
      ...
      !els.remoteVideo.videoWidth ||
      !els.remoteVideo.videoHeight
    ) {
      return;
    }
  ...
  function extractLikelyCardName(text) {
    ...
    cleaned = cleaned
      .replace(
        /\b(ATK|DEF|SPELL|TRAP|MONSTER|EFFECT|CARD|LINK|XYZ|SYNCHRO|FUSION)\b/gi,
        ''
      )
      .replace(/\b\d+\b/g, ' ')
      .replace(/\s+/g, ' ')
